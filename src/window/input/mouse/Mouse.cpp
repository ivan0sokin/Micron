...
namespace Micron
{
    static Array<String, MAX_MOUSE_BUTTONS> buttonsAsStrings;
    String ToString(Button const &button) noexcept { return buttonsAsStrings[EnumToUnsignedSizeType(button)]; }
    ...
    static Void FillButtonsAsStringsArray()
    {
        ...
        {
            buttonsAsStrings[i] = ToString(i) + String("-th");
        }

        for (USize i = 20; i < 60; i += 10)
        {
            buttonsAsStrings[i] = ToString(i) + String("-st");
            buttonsAsStrings[i + 1] = ToString(i + 1) + String("-nd");
            buttonsAsStrings[i + 2] = ToString(i + 2) + String("-rd");
        }
    }
